...
//https://pay.weixin.qq.com/wechatpay_guide/webbased_payment.shtml
//静态二维码 微信 后台重定向到此页面，重定向时，url会附上参数
//使用pay.latipay.net/pay而非spotpay.latipay.net是因为微信商户支付目录目前为pay.latipay.net/pay
import React from 'react';
import URL from 'url-parse';
import numeral from 'numeral';

import { isWechat, hideAds } from '../server/util';
...
class WechatInApp extends React.Component {
  constructor(props) {
    ...
    const hasOrderInfo = !hasAnyEmpty(query, [
      'orderId',
      'createDate',
      'amount',
      'currency',
    ...
    const hasPayment = !hasAnyEmpty(query, [
      'appId',
      'timeStamp',
      'nonceStr',
      'package',
    ...
    const valide = hasOrderInfo && hasPayment && !isEmpty(query.openId);

    if (isWechat) {
      const obj = {
        status: valide ? 'pending' : 'invalid'
      };
  ...
  componentDidMount() {
    ...
    const { paymentData, order } = transaction;
    const { backPageUrl } = order;

    const onBridgeReady = () => {
      window.WeixinJSBridge.invoke('getBrandWCPayRequest', paymentData, res => {
        const { err_msg } = res;
        const map = {
        ...
        if (status === 'failed' || status === 'cancel') {
          ...
          if (
            ...
            this.redirectToMerchantFromWechatJS(
              staticConfirmation,
              status,
              err_msg
            );
  ...
  redirectToMerchantFromWechatJS = (staticConfirmation, status) => {
    const { transaction } = this.state;
    const { order } = transaction;
    const { orderId, currency, amount, createDate, accountCode } = order;

    const url = URL(staticConfirmation, true);
    const query = {
      ...url.query,
      order_id: orderId,
  ...
  render() {
    const { status, needOpenInWechat } = this.state;
