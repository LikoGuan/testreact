...
// transaction
export const genPayData = (
  { wallet_id, amount, payment_method, merchant_reference },
  profile
) => {
  const { user_id, api_key } = profile;
  const payload = {
    wallet_id,
    amount,
    payment_method,
  ...
  const signature = CryptoJS.HmacSHA256(msg, api_key).toString();
  return {
    ...payload,
    signature
  };
...
export const makeForm = (endpoint, payload) => {
